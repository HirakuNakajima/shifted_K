...
\crefformat{section}{\S#2#1#3} % to remove unnecessary space
...
\subsection{Algebraic symplectic reductions}\label{Asr}
...
\subsection{Apr.~17, 2022}

Let $(X,A,B,\xi)$ be a critical point of the perturbed potential. Then $[\xi,X] = 0$, $\xi A = A\Xi$,
$B\xi = \Xi B$. We consider the variety of subspaces $S$ of $V$ such that $X(S)\subset S$, $S\supset
A(W)$. This variety is invariant under the action of $\exp(t\xi)$: e.g.,
\begin{equation*}
\begin{gathered}
\exp(t\xi)S\supset \exp(t\xi)A(W) = A \exp(t\Xi) W= A(W),\\
X(\exp(t\xi)S) = \exp(t\xi)\exp(-t\xi)X\exp(t\xi) S = \exp(t\xi) X S \subset \exp(t\xi)S.
\end{gathered}
\end{equation*}


The case of $B(S) = 0$ is similar. Since this variety is a closed subvariety of the Grassmannian 
(hence projective), there is a fixed point of $\exp(t\xi)$ if it is nonempty, i.e., a subspace $S$ such that $\exp(t\xi) S = S$, i.e. $\xi(S)\subset S$. Therefore $(X,A,B,\xi)$ is $\zeta$-stable if and only if $(X,A,B)$ is $\zeta$-stable.
...
\subsection{When $V$ grows with $L$: singularities in the $R$-matrix}\label{subsec:Vgrow}
...
\bibliography{shifted_K,nakajima,mybib}
\end{document}
